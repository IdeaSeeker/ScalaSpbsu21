...
import org.spbsu.mkn.scala.MyGenericList.{fromSeq, size, sort, sum, toSeq}
...
class MyGenericListTest extends AnyFunSuite {
    ...
    test("sort") {
        ...
        assert(sort(fromSeq(Seq("b", "a", "c"))) == fromSeq(Seq("a", "b", "c")))
        assert(sort(fromSeq(Seq('c', 'b', 'a'))) == fromSeq(Seq('a', 'b', 'c')))
    }
